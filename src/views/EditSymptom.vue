<template>
  <div class="container justify-center items-center mx-auto">
    <h2 class="text-3xl tracking-tight font-extrabold text-violet-500 text-center mt-10 mb-10">
      <span class="block">Edit Symptoms</span>
    </h2>
...
const getSymptom = async () => {
  try {
    await axios
        .get(`https://maria.develop.eiddew.com/licenta/public/api/symptom/${id}`)
        .then((response) => {
          name.value = response.data.symptom.name;
          description.value = response.data.symptom.description;
        });
  } catch (e) {
    console.log(e);
...
const onSubmit = async () => {
  try {
    await axios.put(`https://maria.develop.eiddew.com/licenta/public/api/symptom/${id}`, {
      name: name.value,
